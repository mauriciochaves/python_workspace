...
@step(u'I login with email "{email}"')
...
def step_impl(context, name, job):
    ...
    context.response = post(context.base_url + "users", data=data, token=context.token)


@step(u'I change an employee {employee_id} with name "{name}" and job "{job}"')
def step_impl(context, employee_id, name, job):
    data = generate_data_file(name=name, job=job)
    context.response = put(context.base_url + "users/" + employee_id, data=data, token=context.token)
